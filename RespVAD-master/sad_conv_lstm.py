...
def visualize(h, train_key, val_key, title, y_label, x_label, dir_name):
    ...
    plt.savefig(NO_PROCESSED_DATA_DIR / f'{dir_name}/conv_lstm_sad_model_{train_key}.png')
...
def load_non_overlapped_train_data(base_dir):
    ...
    x, y = train_data[:, :WINDOW_SIZE].reshape(-1, WINDOW_SIZE, 1, 1), train_data[:, WINDOW_SIZE:].reshape(-1,
                                                                                                           WINDOW_SIZE,
                                                                                                           1)
...
def load_non_overlapped_test_data(base_dir):
    ...
    x, y = test_data[:, :WINDOW_SIZE].reshape(-1, WINDOW_SIZE, 1, 1), test_data[:, WINDOW_SIZE:].reshape(-1,
                                                                                                         WINDOW_SIZE, 1)
    cw = {0: np.sum(y) / np.prod(y.shape), 1: 1 - np.sum(y) / np.prod(y.shape)}
    return x, y, cw
...
def load_overlapped_test_data(base_dir):
    ...
    for i, data in enumerate(test_data_list):
        test_x_list.append(np.load(data)[:, :WINDOW_SIZE].reshape(-1, WINDOW_SIZE, 1, 1))
...
# def load_non_overlapped_train_data(base_dir):
#     train_data = np.load(str(NO_PROCESSED_DATA_DIR / base_dir) + '/non_overlapping_seq_train_data.npy')
#     x, y = train_data[:, :WINDOW_SIZE].reshape(-1, WINDOW_SIZE), train_data[:, WINDOW_SIZE:].reshape(-1, WINDOW_SIZE,
#                                                                                                      1)
#     cw = {0: np.sum(y) / np.prod(y.shape), 1: 1 - np.sum(y) / np.prod(y.shape)}
#     return x, y, cw
#
...
# def load_non_overlapped_test_data(base_dir):
#     test_data = np.load(str(NO_PROCESSED_DATA_DIR / base_dir) + '/non_overlapping_seq_test_data.npy')
#     x, y = test_data[:, :WINDOW_SIZE].reshape(-1, WINDOW_SIZE), test_data[:, WINDOW_SIZE:].reshape(-1,
#                                                                                                    WINDOW_SIZE, 1)
#     cw = {0: np.sum(y) / np.prod(y.shape), 1: 1 - np.sum(y) / np.prod(y.shape)}
#     return x, y, cw
#
...
# def load_overlapped_train_data(base_dir):
#     train_data = np.load(str(O_PROCESSED_DATA_DIR / base_dir) + '/overlapping_seq_train_data.npy')
#     x, y = train_data[:, :WINDOW_SIZE].reshape(-1, WINDOW_SIZE), train_data[:, WINDOW_SIZE:].reshape(-1, WINDOW_SIZE, 1)
#     cw = {0: np.sum(y) / np.prod(y.shape), 1: 1 - np.sum(y) / np.prod(y.shape)}
#     return x, y, cw
#
#
...
#     test_y_list = []
#     for i, data in enumerate(test_data_list):
#         test_x_list.append(np.load(data)[:, :WINDOW_SIZE].reshape(-1, WINDOW_SIZE))
#         test_y_list.append(np.load(data)[:, WINDOW_SIZE:].reshape(-1, WINDOW_SIZE, 1))
#     return test_x_list, test_y_list
...
def build_model():
    ...
    model.add(tf.keras.layers.Input(batch_shape=(None, WINDOW_SIZE, SEQ_LEN, 1), name="input"))
    model.add(tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1D(filters=16, kernel_size=5, padding='same',
                                                                     dilation_rate=3, activation='tanh')))
    model.add(tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1D(filters=16, kernel_size=5, padding='same',
                                                                     dilation_rate=3, activation='tanh')))
    # model.add(tf.keras.layers.Reshape((WINDOW_SIZE, 16 * SEQ_LEN)))
    ...
    model.add(tf.keras.layers.TimeDistributed(tf.keras.layers.Dense(32, activation=tf.nn.relu)))
...
# def build_model():
#     model = tf.keras.models.Sequential(name='seq-model')
#     model.add(tf.keras.layers.Input(shape=(WINDOW_SIZE, ), name="input"))
#     model.add(tf.keras.layers.RepeatVector(N_REPEAT))
#     model.add(tf.keras.layers.Reshape((WINDOW_SIZE, N_REPEAT, 1)))
#     model.add(tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1D(filters=32, kernel_size=3, padding='same',
#                                                                      dilation_rate=1, activation='tanh')))
#     model.add(tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1D(filters=32, kernel_size=3, padding='same',
#                                                                      dilation_rate=1, activation='tanh')))
#     model.add(tf.keras.layers.TimeDistributed(tf.keras.layers.Flatten()))
#     model.add(tf.keras.layers.TimeDistributed(tf.keras.layers.Dense(64, activation=tf.nn.relu)))
#     model.add(tf.keras.layers.TimeDistributed(tf.keras.layers.Dense(128, activation=tf.nn.relu)))
#     model.add(tf.keras.layers.Bidirectional(tf.compat.v1.keras.layers.CuDNNLSTM(units=256, return_sequences=True),
#                                             merge_mode='concat'))
#     model.add(tf.keras.layers.TimeDistributed(tf.keras.layers.Dense(1, activation='sigmoid')))
#     return model
...
def conv_lstm_sad_non_overlapped(bs, n_epoch, dir_name, exp_no):
    sad_model = build_model()
    h = compile_train_model(sad_model, bs, n_epoch, dir_name, 5e-4, False)
    ...
    create_directory(str(NO_PROCESSED_DATA_DIR / dir_name) + '/trained_models/conv_lstm_sad_non_overlapped')
    sad_model.save(
        str(NO_PROCESSED_DATA_DIR / dir_name) + f'/trained_models/conv_lstm_sad_non_overlapped/model_{exp_no}.h5')

    return roc_auc, acc_s, f1_s, p_s, r_s


def conv_lstm_sad_overlapped(bs, n_epoch, dir_name, exp_no):
    sad_model = build_model()
    h = compile_train_model(sad_model, bs, n_epoch, dir_name, 1e-4,
                            True)  # learning rate = 1e-3 for repeat vector model
    ...
    PREDICTION_DIR = O_PROCESSED_DATA_DIR / f'{dir_name}/predicted/sad_conv_lstm/'
    ...
    create_directory(str(O_PROCESSED_DATA_DIR / dir_name) + '/trained_models/conv_lstm_sad_overlapped')
    sad_model.save(str(O_PROCESSED_DATA_DIR / dir_name) + f'/trained_models/conv_lstm_sad_overlapped/model_{exp_no}.h5')
...
for dataset in ['data3']:
    ...
    for step in range(num_exp):
        v, w, x, y, z = conv_lstm_sad_non_overlapped(bs=BATCH_SIZE, n_epoch=EPOCH,
    ...
    df['Precision'] = p_buf
    df['Recall'] = r_buf
    df.to_csv(str(NO_PROCESSED_DATA_DIR / dataset) + '/conv_lstm_sad_non_overlapped.csv')
...
#     r_buf = []
#     for step in range(num_exp):
#         v, w, x, y, z = conv_lstm_sad_overlapped(bs=BATCH_SIZE, n_epoch=5,
#                                                  dir_name=dataset, exp_no=step) # n_epoch = 3 for repeat vector model
#         roc_auc_buf.append(v)
#         acc_buf.append(w)
#         f1_buf.append(x)
#         p_buf.append(y)
...
#     df['Precision'] = p_buf
#     df['Recall'] = r_buf
#     df.to_csv(str(O_PROCESSED_DATA_DIR / dataset) + '/conv_lstm_sad_overlapped.csv')
