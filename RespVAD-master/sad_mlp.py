...
def visualize(h, train_key, val_key, title, y_label, x_label, dir_name):
    ...
    plt.savefig(NO_PROCESSED_DATA_DIR / f'{dir_name}/mlp_sad_model_{train_key}.png')
...
def load_non_overlapped_test_data(base_dir):
    ...
    x, y = test_data[:, :WINDOW_SIZE].reshape(-1, WINDOW_SIZE), test_data[:, WINDOW_SIZE:].reshape(-1, WINDOW_SIZE, 1)
    cw = {0: np.sum(y) / np.prod(y.shape), 1: 1 - np.sum(y) / np.prod(y.shape)}
    return x, y, cw
...
def build_model():
    ...
    # model.add(tf.keras.layers.Reshape((WINDOW_SIZE, N_REPEAT)))
...
def mlp_sad_non_overlapped(bs, n_epoch, dir_name, exp_no):
    sad_model = build_model()
    h = compile_train_model(sad_model, bs, n_epoch, dir_name, 1e-4, False)
    ...
    create_directory(str(NO_PROCESSED_DATA_DIR / dir_name) + '/trained_models/mlp_sad_non_overlapped')
    sad_model.save(str(NO_PROCESSED_DATA_DIR / dir_name) + f'/trained_models/mlp_sad_non_overlapped/model_{exp_no}.h5')

    return roc_auc, acc_s, f1_s, p_s, r_s


def mlp_sad_overlapped(bs, n_epoch, dir_name, exp_no):
    sad_model = build_model()
    ...
    PREDICTION_DIR = O_PROCESSED_DATA_DIR / f'{dir_name}/predicted/sad_mlp/'
    ...
    create_directory(str(O_PROCESSED_DATA_DIR / dir_name) + '/trained_models/mlp_sad_overlapped')
    sad_model.save(str(O_PROCESSED_DATA_DIR / dir_name) + f'/trained_models/mlp_sad_overlapped/model_{exp_no}.h5')
...
#         v, w, x, y, z = mlp_sad_non_overlapped(bs=BATCH_SIZE, n_epoch=EPOCH,
...
#     df['Precision'] = p_buf
#     df['Recall'] = r_buf
#     df.to_csv(str(NO_PROCESSED_DATA_DIR / dataset) + '/mlp_sad_non_overlapped.csv')

num_exp = 10
# for dataset in ['data1', 'data2', 'data3', 'data4']:
for dataset in ['data4']:
    df = pd.DataFrame({'Exp. No.': [i for i in range(num_exp)]})
    roc_auc_buf = []
    acc_buf = []
    f1_buf = []
    ...
    r_buf = []
    for step in range(num_exp):
        v, w, x, y, z = mlp_sad_overlapped(bs=2048, n_epoch=5,
                                           dir_name=dataset, exp_no=step)
        roc_auc_buf.append(v)
        acc_buf.append(w)
        f1_buf.append(x)
    ...
    df['Precision'] = p_buf
    df['Recall'] = r_buf
    df.to_csv(str(O_PROCESSED_DATA_DIR / dataset) + '/mlp_sad_overlapped.csv')
